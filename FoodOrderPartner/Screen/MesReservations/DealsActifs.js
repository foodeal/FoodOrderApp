  ...
  const getData = async (id) => {
    ...
    await fetch(url)
      ...
      .then(res => {
        ...
        var newdatta = res.coupon.filter(function (item) { return item.status_restaurant == ''  });
        setData({
          ...data,
          data: newdatta.filter((item)=> 
          {
            
            let date = moment().format();
            const datenow = moment(date).format("YYYY-MM-DD")
            const createtime = moment(item.createdAt).format("YYYY-MM-DD")
            const sameee = moment(createtime).isSame(datenow)
            if(sameee){
              return item
            }
          }
      ...
      <View style={styles.Container}>
        {data.data.length == 0 ?
          <View style={{ justifyContent: 'center', alignItems: 'center', flex: 1 }}>
            ...
            <Text style = {{width: wp('60%'), textAlign:'center', color:'#686663'}}> Baskets that have not yet been collected can be found here </Text>
          </View>
          :
          <FlatList
