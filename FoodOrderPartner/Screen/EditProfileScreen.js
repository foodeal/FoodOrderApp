...
const EditProfileScreen = ({ navigation,route }) => {
  ...
  const [image, setImage] = useState(data.data.logourl);



  const update = () => {
    if (mail == ""  || url == "" || address == "" || phone == "") {
      Toast.show('You must fill in all the fields')
    }
    else {
      axios
        .put(`${config.url}/restaurants/${data.data.restaurant_id}`, {
          mail: mail,
          url: url,
          address: address,
          phone: phone
        })
        .then(res => { if (res.data == 'Parnter Updated!') { Toast.show('Your profile has been updated successfully') } })
  ...
  return (
    <View style={styles.container}>
      <View style={{ height: Platform.OS  === 'ios' ? hp('10%'):hp('8%'), backgroundColor: 'white', elevation: 3 }}>
            ...
            <TouchableOpacity>
              <View
                ...
                {data.data.logourl == null ?
                  ...
                  <ImageBackground
                    source={{ uri: image }}
                    style={{ height: hp('16%'), width: wp('29%'),}}
                    imageStyle={{ borderRadius: 15,borderWidth:1 }}>
                  </ImageBackground>
                }
              </View>
            </TouchableOpacity>
